# Gundam Char's Counterattack Launcher
...
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File .\launcher\gundamChar.ps1
...
#     %APPDATA%\GundamCharRecomp\launcher-config.json (per-user). Falls back
...
[CmdletBinding()]
param(
    [string]$RecompExeName = 'Gundam_Char_s_Counterattack_Recompiled.exe'
)
...
function Get-ConfigDir {
    ...
    $dir = Join-Path $appData 'GundamCharRecomp'
...
$form.Text = 'Gundam Char Counterattack Launcher'
...
$lblInfo.Text = "Select your legally obtained Gundam Char's Counterattack disc image (.cue).
...
$btnBrowse.Add_Click({
    ...
    $dlg.Title = 'Select your legally obtained Gundam Char Counterattack disc image'
...
function Show-Error {
    ...
    [System.Windows.Forms.MessageBox]::Show($form, $Message, 'Gundam Char Counterattack Launcher', 'OK', 'Warning') | Out-Null
}
