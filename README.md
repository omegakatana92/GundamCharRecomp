# Mobile Suit Gundam: Char's Counterattack - Recomp
...
A modern fan-made recompilation project for **Mobile Suit Gundam: Char's Counterattack**, originally released for the **Sony PlayStation**.
...
To play, you must provide your **own legally obtained copy** of *Mobile Suit Gundam: Char's Counterattack*.
...
* `launcher\gundamChar.bat` â€” double-click to start.
* `launcher\gundamChar.ps1` â€” the actual PowerShell + WinForms GUI.

The launcher lets you:

1. Pick your legally obtained Gundam Char's Counterattack `.cue` file (Browse...).
...
`%APPDATA%\%GundamCharRecomp\launcher-config.json` and re-loaded the next time
the launcher starts.
...
```bash
git clone https://github.com/omegakatana92/GundamCharRecomp.git
cd GundamCharRecomp
```
...
**Mobile Suit Gundam: Char's Counterattack**, its characters, names, artwork, music, trademarks, and other intellectual property remain the property of their respective copyright and trademark holders.
